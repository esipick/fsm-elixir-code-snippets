defmodule Fsm.Scheduling do
  ...
  def delete_appointment(
    ...
    context = %{
      ...
      school_id: school_id,
      params: %{"school_id" => to_string(school_id)},
      request_path: "/api/appointments"
    }

    if Flight.Auth.Authorization.user_can?(
         current_user,
      ...
      if Scheduling.Appointment.is_paid?(appointment) or current_user.archived do
        message =
          "Paid appointment cannot be deleted! " <>
            "Please contact administrator to re-schedule or delete the content."
        ...
        instructor_user_id = if instructor_user_id == "", do: nil, else: instructor_user_id
        ...
        if Flight.Auth.Authorization.user_can?(
          ...
          {:error, "Appointment associated with other users cannot be deleted."}
        end
      end
    end
  end
  ...
  def update_appointment(
    ...
    if !Flight.Auth.Authorization.user_can?(
         current_user,
         [Permission.new(:appointment, :modify, :all)]
       ) and Scheduling.Appointment.is_paid?(appointment) do
      message =
        "Paid appointment cannot be modified! " <>
          "Please contact administrator to re-schedule or delete the content."

      {:error, message}
    else
      if current_user.archived do
        {:error, "You can't update appointment at the moment, Please contact administrator."}
      else
  ...
  def send_changed_notifications(appointment, modifying_user) do
    ...
    if appointment.user_id and modifying_user.id != appointment.user_id do
    ...
    if appointment.instructor_user and appointment.instructor_user.id != modifying_user.id do
      Flight.PushNotifications.appointment_changed_notification(
  ...
  def upsert_appointment(
    ...
    if changeset.valid? do
      ...
      {should_delete_item, changeset} =
        if (appointment.aircraft_id != nil and appointment.aircraft_id != new_aircraft_id) or
             (appointment.simulator_id != nil and appointment.simulator_id != new_simulator_id) do
          changeset =
            changeset
      ...
      case Repo.insert_or_update(changeset) do
        {:ok, appointment} ->
          assign_instructor_to_user =
            user_id not in ["", nil] and instructor_user_id not in ["", nil]

          assign_aircraft_to_user = user_id not in ["", nil] and aircraft_id not in ["", nil]

          if assign_instructor_to_user do
            Repo.insert(%UserInstructor{user_id: user_id, instructor_id: instructor_user_id},
              on_conflict: :nothing
            )
          end

          if assign_aircraft_to_user do
            Repo.insert(%UserAircraft{user_id: user_id, aircraft_id: aircraft_id},
              on_conflict: :nothing
            )
          end
        ...
        other ->
          if Map.get(temp_changeset, :id) != nil and is_create? == true,
  ...
  def create_appointment(
    ...
    if !Flight.Auth.Authorization.user_can?(current_user, [
         Permission.new(:appointment, :create, :all)
       ]) or
         current_user.archived do
      {:error, "You can't create appointment at the moment. Please contact administrator"}
    else
      case insert_or_update_appointment(
             %Appointment{},
             appointment_data,
  ...
  def insert_or_update_unavailability(
        %{context: %{current_user: %{school_id: school_id, id: user_id}}} = context,
        unavailability,
        attrs
      ) do
    school = Fsm.SchoolScope.get_school(school_id)
    ...
    school_context = %Plug.Conn{
      assigns: %{current_user: current_user},
      params: %{"school_id" => inspect(school_id)},
      request_path: "/api/"
    }
        ...
        if aircraft_id do
          ...
          case status do
            ...
            :unavailable ->
              ...
              add_error(changeset, key, "already have unavailability within the given time range",
                status: status
              )
          ...
          case status do
            ...
            :unavailable ->
              add_error(
                changeset,
                :room,
                "already have unavailability within the given time range",
                status: status
              )
